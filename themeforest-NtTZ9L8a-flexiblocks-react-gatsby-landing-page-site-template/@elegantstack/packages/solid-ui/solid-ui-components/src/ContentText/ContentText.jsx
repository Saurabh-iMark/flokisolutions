...
const ContentText = ({ as: CustomComponent, content, children, ...props }) => {
  ...
  return contentArray.map(({ text, ...contentRest }, index) => {
    ...
    let textWithBreak = null

    if (isHeading && !children) {
      textWithBreak = text.split(/\*{2}(.*?)\*{2}/gi)
      for (var i = 1; i < textWithBreak.length; i += 2) {
        textWithBreak[i] = (
    ...
    return isHeading ? (
      <Heading
        ...
        {...mergedProps}
      >
        { textWithBreak|| children || textWithSpecial || text || textWithBreak}
      </Heading>
    ) :variant==='link'?
    <a href='mailto:omar@flokisolutions.com'>{text}</a>
   :(
      <Text
        key={`item-${index}`}
        variant={variant}
